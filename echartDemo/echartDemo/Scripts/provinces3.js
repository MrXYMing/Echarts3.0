...
function SetSeries(_seriesName) {
    ...
    for (var x in _provinces) {
        for (y in _provinces[x].solid) {
            ...
            pointData.push({ "name": _provinces[x].solid[y].to, "value": _provinces[x].solid[y].count });
    ...
    var lineDatas = [[_seriesName, lineData]];
    var dotLineDatas = [['虚线', dotLineData]];
...
function SetCityOption(_selectedProvince) {
    myChart.showLoading({ text: "数据加载中......" });
    //全省城市坐标
    citys = GetCitys(_selectedProvince);
    //全省干线
    citysTrunk = GetCityTrunk(_selectedProvince);
    var series = SetCitySeries(_selectedProvince, "全省");
    option = {
    ...
    $(".box").on("change", "#legend", function () {
        ...
        var _seriesName = $(this).val();
        var _option = myChart.getOption();
        _option.series = SetCitySeries(selectedProvince, _seriesName);
        _option.legend = {
            show: false,
            orient: 'vertical',
            x: 'left',
...
function SetCitySeries(_selectedProvince, _selectedCity) {
    ...
    lineDatas = [[_selectedProvince, lineData]];
...
function SetChina(_selectedProvince) {
    ...
    myChinaChart.on("mapselectchanged", function (param) {
        ...
        //全省城市坐标
        citys = GetCitys(selectedProvince);
        //全省干线
        citysTrunk = GetCityTrunk(selectedProvince);
        var _option = myChart.getOption();
        _option.series = SetCitySeries(selectedProvince, "全省");
        _option.dataRange = {
            //显示策略
            show: true,
            //指定的最小值，eg: 0，默认无，必须参数，唯有指定了splitList时可缺省min。
...
function ShowWaybill(_name) {
    ...
    if (res.IsExist) {
        ...
        html = "<div><span>" + res.message + "</span></div>";
    }
...
function GetProvinces() {
    ...
    action.ActionUrl = "http://localhost:17463/echartService.asmx/GetContryWarehouse";
...
function GetProvincesTrunk() {
    var result = [];
    var action = new Action();
    action.ActionUrl = "http://localhost:17463/echartService.asmx/GetContryMainTransport";
    action.Done = function (res) {
        console.log(res);
        result = res.d;
    }
...
//获取全国城市
function GetCitys(_province) {
    var result = {};
    var action = new Action();
    action.ActionData = JSON.stringify({ province: _province });
    action.ActionUrl = "http://localhost:17463/echartService.asmx/GetProvinceWarehouse";
    action.Done = function (res) {
        console.log(res);
        var data = res.d;
        for (x in data) {
...
function GetCityTrunk(_selectedProvince) {
    ...
    action.ActionUrl = "http://localhost:17463/echartService.asmx/GetProvinceTransport";
    action.Done = function (res) {
        console.log(res);
        result = res.d;
    }
...
//获取运单清单
function GetWaybillList(_path) {
    var result = {};
    result["IsExist"] = true;
    var action = new Action();
    action.ActionData = JSON.stringify({ path: _path });
    action.ActionUrl = "http://localhost:17463/echartService.asmx/GetTransportOrders";
    action.Done = function (res) {
        console.log(res);
...
function GetDetails(_WaybillNo) {
    ...
    action.ActionUrl = "http://localhost:17463/echartService.asmx/GetTransportOrderInfor";
    action.Done = function (res) {
        console.log(res);
        var data = res.d;
        var details = [];
