...
$S.SetOption = function (_seriesName) {
    ...
    action.Done = function (res) {
        ...
        action.Done = function (res) {
            //全国总仓干线
            $S.myChinaChartTrunk = res.d;

            var trunkList = ["全国"];
            for (var x in $S.myChinaChartTrunk) {
                trunkList.push($S.myChinaChartTrunk[x].name);
            }
...
$S.SetSeries = function (_seriesName) {
    ...
    var series = $S.GetSeries(_seriesName, _provinces, $S.myChinaChart);
...
$S.SetCityOption = function (_selectedProvince) {
    ...
    action.Done = function (res) {
        ...
        action.Done = function (res) {
            ...
            $S.SetCitySeries(_selectedProvince, "全省");
            ...
            $(".box").on("click", "#legend li", function () {
                ...
                $S.legend.hide();
                $(".legendbox .styled-select").html(_seriesName);
                $S.SetCitySeries($S.selectedProvince, _seriesName);
            });
    ...
    $S.myChinaChart.on("mapselectchanged", function (param) {
        ...
        action.Done = function (res) {
            ...
            action.Done = function (res) {
                ...
                $S.SetCitySeries($S.selectedProvince, "全省");
...
$S.GetTrunk = function (_name) {
    var trunk = [];
    if (_name === "全国") {
        trunk = $S.myChinaChartTrunk;
    } else {
        for (var x in $S.myChinaChartTrunk) {
            if ($S.myChinaChartTrunk[x].name === _name) {
                trunk.push($S.myChinaChartTrunk[x]);
                break;
            }
        }
    }
